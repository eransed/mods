...
fn parse_incoming_message(text: &str) -> Option<Message> {
  serde_json::from_str::<IncomingTopicMessage>(text).ok().and_then(|message| {
    match message.topic.as_str() {
      "ping" => Some(Message::Ping { sender: "ws_client" }),
      "pong" => Some(Message::Pong { sender: "ws_client" }),
      _ => None,
    }
  })
}
...
fn encode_topic_message(message: &Message) -> Option<String> {
  match message {
    Message::Pong { sender } => {
      serde_json::to_string(&TopicMessage { topic: "pong".to_string() }).ok().map(|json| {
      ...
      while let Ok(message) = receiver.recv().await {
        match message {
          ...
          Message::Ping { .. } => {
            ...
            let _ = broadcast_sender.send(Message::Pong { sender: "ws_server" });
          }
      ...
      tokio::spawn(async move {
        ...
        let write_task = tokio::spawn(async move {
          while let Some(message) = rx.recv().await {
            trace!("Sending {} bytes to {}", message.len(), client_addr);
        ...
        info!("ws_server websocket client connected: {}, local: {}", client_addr, local_addr);
          ...
          match message_result {
            Ok(WsMessage::Text(text)) => {
              if let Some(message) = parse_incoming_message(&text) {
                ...
                info!(text = %text, "ws_server received websocket topic message: {}", client_addr);
              } else {
                let broadcast_message = Message::Broadcast { sender: name, body: text.clone() };
...
mod tests {
  ...
  fn parses_ping_payloads_from_json() {
    ...
    assert_eq!(message, Some(Message::Ping { sender: "ws_client" }));
  }

  #[test]
  fn encodes_pong_payloads_for_clients() {
    let encoded = encode_topic_message(&Message::Pong { sender: "ws_server" });
