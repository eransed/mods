...
const MF_0004_MID_NUMBER: MidField = MidField { name: "mid_number", rng: 20..24 };

const MF_0004_ERROR_CODE: MidField = MidField { name: "error_code", rng: 24..26 };
...
pub fn mid_parse_0004(data: &str) -> Result<Mid0004, String> {
  ...
  if m4.header.mid != 4 {
    return Err(format!("Unexpected mid {} when parsing for mid 4", m4.header.mid));
  }
...
mod tests {
  ...
  fn mid_0004_name() {
    assert_eq!(
      MidName::from_repr(4).unwrap(),
      MidName::from_str("Application Communication negative acknowledge").unwrap()
    );
  }
