...
const MF_LEN: MidField = MidField {
    name: "len",
    rng: 0..4,
};

const MF_MID: MidField = MidField {
    name: "mid",
    rng: 4..8,
};

const MF_REV: MidField = MidField {
    name: "rev",
    rng: 8..11,
};

const MF_NO_ACK_FLAG: MidField = MidField {
    name: "no_ack_flag",
    rng: 11..12,
};

const MF_STATION_ID: MidField = MidField {
    name: "station_id",
    rng: 12..14,
};

const MF_SPINDLE_ID: MidField = MidField {
    name: "spindle_id",
    rng: 14..16,
};

const MF_SEQUENCE_NUMBER: MidField = MidField {
    name: "sequence_number",
    rng: 16..18,
};

const MF_NUMBER_OF_MESSAGE_PARTS: MidField = MidField {
    name: "number_of_message_parts",
    rng: 18..19,
};

const MF_MESSAGE_PART_NUMBER: MidField = MidField {
    name: "message_part_number",
    rng: 19..20,
};
...
pub fn mid_parse_header(raw_mid: &str) -> Result<MidHeader, String> {
    ...
    if header.len < 20 {
        return Err(format!(
            "Invalid length '{}' when parsing the mid header",
            header.len
        ));
    }
    ...
    if header.mid < 1 || header.mid > 9999 {
        return Err(format!(
            "Invalid mid '{}' when parsing the mid header",
            header.mid
        ));
    }
    ...
    if header.rev < 1 || header.rev > 9999 {
        return Err(format!(
            "Invalid revision '{}' when parsing the mid header",
            header.rev
        ));
    }
...
mod tests {
    ...
    fn mid_header_invalid_1_error() {
        let _ = mid_parse_header(
            "sl;dfkjashdgasklfdj;adf;aklsdfhghsldkfjasbcvjldsjflasjdflaksdfa;lksdjf",
        )
        .unwrap();
    }
}
