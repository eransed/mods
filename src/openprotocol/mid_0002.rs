...
const MF_0002_REV1_CELL_ID: MidField = MidField { name: "cell_id", rng: 22..26 };

const MF_0002_REV1_CHANNEL_ID: MidField = MidField { name: "channel_id", rng: 28..30 };

const MF_0002_REV1_CONTROLLER_NAME: MidField = MidField { name: "controller_name", rng: 32..57 };

const MF_0002_REV2_SUPPLIER_CODE: MidField = MidField { name: "supplier_code", rng: 59..62 };

const MF_0002_REV3_OPEN_PROTOCOL_VERSION: MidField =
  MidField { name: "open_protocol_version", rng: 64..83 };

const MF_0002_REV3_CONTROLLER_SOFTWARE_VERSION: MidField =
  MidField { name: "controller_software_version", rng: 85..104 };

const MF_0002_REV3_TOOL_SOFTWARE_VERSION: MidField =
  MidField { name: "tool_software_version", rng: 106..125 };

const MF_0002_REV4_RBU_TYPE: MidField = MidField { name: "rbu_type", rng: 127..151 };

const MF_0002_REV4_CONTROLLER_SERIAL_NUMBER: MidField =
  MidField { name: "controller_serial_number", rng: 153..163 };

const MF_0002_REV5_SYSTEM_TYPE: MidField = MidField { name: "system_type", rng: 165..168 };

const MF_0002_REV5_SYSTEM_SUB_TYPE: MidField = MidField { name: "system_sub_type", rng: 170..173 };

const MF_0002_REV6_SEQUENCE_NUMBER_SUPPORT: MidField =
  MidField { name: "sequence_number_support", rng: 175..176 };

const MF_0002_REV6_LINKING_HANDLING_SUPPORT: MidField =
  MidField { name: "linking_handling_support", rng: 178..179 };

const MF_0002_REV6_STATION_ID: MidField = MidField { name: "station_id", rng: 181..191 };

const MF_0002_REV6_STATION_NAME: MidField = MidField { name: "station_name", rng: 193..218 };

const MF_0002_REV6_CLIENT_ID: MidField = MidField { name: "client_id", rng: 220..221 };
...
impl Mid for Mid0002 {
  fn str(&self) -> String {
    ...
    if rev == 1 {
      ...
      s.push_str(format!("01{:0ciw$}", self.rev1.cell_id).as_str());
      s.push_str(format!("02{:0chi$}", self.rev1.channel_id).as_str());
      s.push_str(format!("03{:0cni$}", self.rev1.controller_name).as_str());
    } else if rev == 2 {
      let sci = MF_0002_REV2_SUPPLIER_CODE.rng.end - MF_0002_REV2_SUPPLIER_CODE.rng.start;
      s.push_str(format!("04{:0sci$}", self.rev2.supplier_code).as_str());
    } else if rev == 3 {
      let opvi =
        MF_0002_REV3_OPEN_PROTOCOL_VERSION.rng.end - MF_0002_REV3_OPEN_PROTOCOL_VERSION.rng.start;
      let csvi = MF_0002_REV3_CONTROLLER_SOFTWARE_VERSION.rng.end
        - MF_0002_REV3_CONTROLLER_SOFTWARE_VERSION.rng.start;
      let tsvi =
        MF_0002_REV3_TOOL_SOFTWARE_VERSION.rng.end - MF_0002_REV3_TOOL_SOFTWARE_VERSION.rng.start;
      s.push_str(format!("05{:0opvi$}", self.rev3.open_protocol_version).as_str());
      s.push_str(format!("06{:0csvi$}", self.rev3.controller_software_version).as_str());
      s.push_str(format!("07{:0tsvi$}", self.rev3.tool_software_version).as_str());
    } else if rev == 4 {
      let rbuti = MF_0002_REV4_RBU_TYPE.rng.end - MF_0002_REV4_RBU_TYPE.rng.start;
      let csni = MF_0002_REV4_CONTROLLER_SERIAL_NUMBER.rng.end
        - MF_0002_REV4_CONTROLLER_SERIAL_NUMBER.rng.start;
      s.push_str(format!("08{:0rbuti$}", self.rev4.rbu_type).as_str());
      s.push_str(format!("09{:0csni$}", self.rev4.controller_serial_number).as_str());
    } else if rev == 5 {
      ...
      s.push_str(format!("10{:sti$}", self.rev5.system_type).as_str());
      s.push_str(format!("11{:ssti$}", self.rev5.system_sub_type).as_str());
    } else if rev == 6 {
      let snsi = MF_0002_REV6_SEQUENCE_NUMBER_SUPPORT.rng.end
        - MF_0002_REV6_SEQUENCE_NUMBER_SUPPORT.rng.start;
      let lhsi = MF_0002_REV6_LINKING_HANDLING_SUPPORT.rng.end
        - MF_0002_REV6_LINKING_HANDLING_SUPPORT.rng.start;
      ...
      s.push_str(format!("12{:0snsi$}", self.rev6.sequence_number_support).as_str());
      s.push_str(format!("13{:0lhsi$}", self.rev6.linking_handling_support).as_str());
      s.push_str(format!("14{:0stai$}", self.rev6.station_id).as_str());
...
pub fn mid_parse_0002(data: &str) -> Result<Mid0002, String> {
  ...
  if m2.header.mid != 2 {
    return Err(format!("Unexpected mid {} when parsing for mid 2", m2.header.mid));
  }
...
pub fn mid_parse_0002_rev2(data: &str) -> Result<Mid0002Rev2, String> {
  let mut m2r2 = Mid0002Rev2 { supplier_code: String::new() };
...
pub fn mid_parse_0002_rev4(data: &str) -> Result<Mid0002Rev4, String> {
  let mut m2r4 = Mid0002Rev4 { rbu_type: String::new(), controller_serial_number: String::new() };
...
pub fn mid_parse_0002_rev5(data: &str) -> Result<Mid0002Rev5, String> {
  let mut m2r5 = Mid0002Rev5 { system_type: String::new(), system_sub_type: String::new() };
...
mod tests {
  ...
  #[test]
  fn mid2_rev2_valid() {
    let _ = mid_parse_0002_rev2(
      "0123000200200000asfadjfhe0uhiudlkjsdfalsdfjldfjajfdjflksdhgsdfg2143123412341234safdasdfasdf",
    )
    .unwrap();
  }
