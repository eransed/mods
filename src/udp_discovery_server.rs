...
impl DiscoveryServer {
    ...
    pub async fn new(
        ...
        let local_peer_info =
            PeerInfo::new(local_node_name, local_ip, local_http_port, system_type);
    ...
    async fn setup_socket() -> Result<tokio::net::UdpSocket, String> {
        ...
        std_socket
            .join_multicast_v4(
                ...
                &"0.0.0.0"
                    .parse()
                    .map_err(|e| format!("0.0.0.0 parse error: {}", e))?,
            )
        ...
        info!(
            "UDP multicast socket setup complete: {}",
            Self::MULTICAST_ADDR
        );
        Ok(tokio_socket)
    }
    ...
    async fn send_discovery_packet(&self) -> Result<(), String> {
        ...
        debug!(
            "Sent discovery beacon for {}",
            self.local_peer_info.node_name
        );
        Ok(())
    }
